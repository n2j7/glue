...
DEFAULT_SETTINGS = {
    ...
    'cachebuster-img-filename': False,
    'global_template':
...
class Sprite(object):
    ...
    def image_path(self, ratio=1, full=True):
        ...
        :param ratio: Ratio.
        """
        filename = '%s%s.png' % (self.image_filename, reference)
        if full:
...
def main():
    ...
    group.add_option("--cachebuster-img-filename", dest="cachebuster_img_filename",
            action='store_true',
            help=("append the sprite's sha first 6 characters "
                  "to the otput image filename only"))
    parser.add_option_group(group)
    ...
    if options.cachebuster and options.cachebuster_filename or \
        options.cachebuster and options.cachebuster_img_filename or \
        options.cachebuster_filename and options.cachebuster_img_filename:
        parser.error("You can't use --cachebuster, "
                     "--cachebuster-filename and --cachebuster-img-filename "
                     "at the same time.")
